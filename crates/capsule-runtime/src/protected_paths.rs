...
//! | A destination whose write-ness depends on another input's value — a `repo` a `git` tool writes under `checkout` and reads under `log` | Neither a `format` value nor [`crate::tool_annotations::KEYWORD_DESTINATIONS`] takes a condition, so such a property stays unannotated and falls back on its name. `repo` is in neither [`TOOL_PATH_KEYS`] nor [`TOOL_DESTINATION_KEYS`], so it is not checked here at all; a conditional destination whose name *is* in one of those tables is judged by key name in both directions. Either outcome is deliberate. |
