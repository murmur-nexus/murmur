...
The sealed-containment procedure is not advisory. The escape-conformance harness
(`crates/capsule-runtime/escape-conformance/`) grades each of its cases against a per-class
expectation, and the `sealed` column of every case is marked `Expectation::Documented` — the
expected verdict is recorded and reported, and **gates nothing**. The variant's own doc-comment names
`docs/content/reference/sealed-containment-manual-verification.md` as the reason: grading a release
on a column nobody has validated against a real composed root would be exactly the false assurance
the harness exists to prevent.

Promoting that column to a gating `Expectation::Must` belongs to whoever runs the manual procedure
...
They stay at those paths deliberately: code comments and test doc-comments across `capsule-runtime`,
`murmur-cli`, and `murmur-artifact` cite them by path as the reason a given piece of enforcement is
