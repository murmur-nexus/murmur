...
skips. Either outcome reports on a boundary the run never touched.

So the properties that only a real host can exhibit are checked on one, by hand, with the observed
output recorded in the procedure itself.

The automated tests that do exist assert the *decision* logic — which enforcement tier a probe
resolves to, which containment class a tier achieves, that a limit value is rejected when it is
zero. None of them touches a kernel boundary.
