...
!!! warning "Kernel enforcement is Linux-only, and that is permanent"
    Every containment claim a *kernel* backs requires Linux with kernel ≥5.13.
...
**Linux with kernel ≥5.13 is the only platform with kernel enforcement.** Landlock, seccomp and cgroups are Linux kernel primitives with no
...
[`W-SEC-009`](diagnostics.md#w-sec-009)). The workdir grant is *not* the full Landlock right-set: character-device
...
[`capabilities.filesystem.workdir_exec: true`](#field-workdir-exec), the workdir
...
`/var/run/docker.sock` (see [W-SEC-005](diagnostics.md#w-sec-005)). Also independently of Landlock, the forked
...
The write side of that character device is defined to discard, so there is no state behind it to
reach, corrupt, or read back. The exception is one inode, not `/dev`, not a directory.
...
[`sealed` class](#field-containment) gets a private `/dev` **tmpfs** carrying the
...
Deciding exec in userspace instead — a supervisor comparing a pathname it read out of the
calling process — does not hold: that comparison can be defeated by renaming a binary to an
allowlisted basename, and, per `seccomp_unotify(2)`, raced even when it cannot. Withholding one
Landlock right replaces the whole mechanism.
...
- [Containment class](#field-containment) — what `advisory`, `scoped`, and `sealed`
  each require of the host, and what each one enforces.
- [Host resource limits](resource-limits.md#host-resource-limits) — the `capabilities.resources`
  fields, their defaults, and their platform behavior.
- [Security warnings](diagnostics.md) — the residual gaps the runtime reports at runtime,
