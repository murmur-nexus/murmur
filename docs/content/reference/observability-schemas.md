...
| `lane` | string | `"user"` \| `"peer"` \| `"bg"` — the queue lane the task waited in, derived from `origin`. `"schedule"`, `"event"`, `"completion"` and `"system"` all map to `"bg"`. See [Queue lanes](../concepts/session-loop.md#queue-lanes) |
