...
| 2 | The environment variable `ANTHROPIC_API_KEY` | Read once at launch, with [`W-SEC-026`](diagnostics.md#w-sec-026) |
| — | Neither | `mur run` refuses with `E-MAN-003`, naming both places |

A literal is read once at launch, also with `W-SEC-026`. The re-read cadence, the single retry
