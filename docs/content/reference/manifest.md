...
| Cancellation | [`tasks/cancel`](../how-to/capsules-a2a-messaging.md#cancelling-a-running-task) interrupts the harness the way the driver's `describe()` declares: `stdin-message` writes the launch plan's interrupt bytes to its stdin, `signal-int` sends it `SIGINT`, and `unsupported` kills it outright. A graceful interrupt gives it 10 seconds to end on its own before it is killed. The task is `canceled` either way, and the capsule, its queue and its conversation keep going. |
